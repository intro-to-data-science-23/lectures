...
There are **multiple ways to store the same data in a dataset** (or across multiple tables; but more on that in the next session).
...
- This often implies a trade-off between shortness and uniqueness. In any case, you would try to avoid situations that force you disambiguate functions with the same name (as in `dplyr::select`; see ["R packages"](https://r-pkgs.org/dependencies.html#namespace)).
...
Functions and debugging. Note that the automation and scheduling part has been outsourced, but not abandoned. Stay tuned!
